...
export default function VentaImprimible({ empresa, venta, cliente, items }) {
  ...
  const moneda = venta?.moneda || empresa?.moneda_base || "COP";
    ...
    <div className="print-only" style={{ maxWidth: 720, margin: "24px auto", padding: 24 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", borderBottom: "2px solid #222", paddingBottom: 16, marginBottom: 16 }}>
        ...
        <div style={{ textAlign: "right" }}>
          <div style={{ fontSize: 11, textTransform: "uppercase", opacity: 0.6 }}>Venta</div>
          <div style={{ fontSize: 18, fontWeight: 700 }}>{venta?.id}</div>
          <div style={{ fontSize: 12, opacity: 0.7 }}>{venta?.fecha}</div>
          {venta?.numero_factura && <div style={{ fontSize: 12, opacity: 0.7 }}>Factura {venta.numero_factura}</div>}
        </div>
      </div>
